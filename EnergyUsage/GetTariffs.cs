...
namespace EnergyUsage
{
    public partial class Form1
    {
        private void GetTariffs()
        {
            ...
            string region = "-A";// + cmbobx_regions.SelectedText;
            ...
            Utilities.GetStandingCharge(new Uri("https://api.octopus.energy/v1/products/" + code + tariff + fuel + rate + code + region + type));
            ...
            for (int i = 0; i < myStandingChargeDeserializedData.count; i++)
            {
                if (myStandingChargeDeserializedData.results[i].payment_method == paymentType)
                {
                    ...
                    rchtxtbx_tariff_info.AppendText("Valid From = " +
                                                    myStandingChargeDeserializedData.results[i].valid_from + "\r");
                    rchtxtbx_tariff_info.AppendText("Valid To = " +
                                                    myStandingChargeDeserializedData.results[i].valid_to + "\r");
